...
The annotation applies only to the immediately following call or constructor invocation. It tells WyW that the call does
not mutate its arguments or other state; it does not make the returned value statically analyzable, and calls inside the
arguments are still analyzed separately.

Only add this annotation when that guarantee is true. An incorrect annotation can make extracted CSS use a value that
the runtime call would have changed. The comment must also reach WyW intact, so configure earlier transforms to preserve
it if necessary. When static evaluation fails because of an eligible opaque call, the error includes its file, location,
and the exact annotation placement to consider.
