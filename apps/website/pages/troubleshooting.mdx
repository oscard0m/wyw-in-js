...
### Marking one call as side-effect-free

With `eval.strategy: "static"`, WyW rejects a top-level call when it cannot prove that the call leaves values used by
later style interpolations unchanged. If you know that a particular call is side-effect-free, annotate that invocation
with the conventional `/*#__PURE__*/` or `/*@__PURE__*/` block comment:

```ts
const spacing = { medium: 12 };

const serialized = /*#__PURE__*/ serialize(spacing);
...
The annotation applies only to the following call or constructor invocation, optionally wrapped in parentheses or
TypeScript-only expression wrappers. In a chained invocation it also covers calls used to compute the callee, matching
the convention used by bundlers and minifiers; calls in the annotated invocation's own arguments are still analyzed
separately. It does not apply to a call buried inside a surrounding binary, conditional, or sequence expression. The
annotation tells WyW that the call does not mutate its arguments or other state; it does not make the returned value
statically analyzable.

Only add this annotation when that guarantee is true. An incorrect annotation can make extracted CSS use a value that
the runtime call would have changed. Bundlers and minifiers also recognize this convention and may remove an unused
annotated call, so the call must have no required side effects at runtime either. The comment must reach WyW intact, so
configure earlier transforms to preserve it if necessary. When static evaluation fails because of an eligible opaque
call, the error includes its file, location, and the exact annotation placement to consider.
