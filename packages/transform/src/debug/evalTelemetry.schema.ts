...
export const EVAL_TELEMETRY_SCHEMA = deepFreeze({
  ...
  denominators: {
    ...
    transmission:
      'logicalResults counts complete logical LOAD_RESULT responses and partitions into initial + omissions + resends + serializedExports + errors. moduleResetSignals is orthogonal and counts complete logical results carrying resetModule. Invalidation is a mutually exclusive resend reason only when a prior code shipment exists; resend reason precedence is invalidation, hash change, storage-shape change, then only widening. incompleteResults counts responses whose write failed and is excluded from logicalResults. chunks counts successfully written physical messages carrying codeChunk; chunkedResults counts complete logical responses with chunks. wireMessages and wireBytes count successfully written, already serialized physical messages; telemetry must not serialize transport again.',
  },
  encodings: {
